            ...
            if (obj.length > 0) {
              if (obj[0].ERRORMSG == undefined) {
                ...
                if(HouseObjectToGenerateVT.length >= 1){
                  ...
                  TotalNoP =  parseInt(HouseObjectToGenerateVT[0].AWBPcs);
                  TotalGrWt = parseInt(HouseObjectToGenerateVT[0].GrWt);
  ...
  $('#VTList').find('.panel-group').each(function (index) {
    $(this).find('.vehicleDetails').each(function() {
      ...
      HouseObjectToGenerateVT.forEach((element, i) => {
        ...
        //   sbID = element.SBGUID;
        // }
     CustID = (element.GHAID).toString();
        AssignedAWBdata = {
            "rowIndex":"2",
                      "AWBid":(element.DOID).toString(), //TSM AWBiD
                      "DOId":(element.DOID).toString(), // for import 
                      "HAWBID":(element.HAWBId).toString(), ////HAWB if any TSM HAWBiD
                      "AllocateNOP":element.AWBPcs,
                      "AllocateGrWt":element.GrWt,
                      "GHAID":(element.GHAID).toString() //TSM Branch ID
          };
...
if(r.test(v)) {
  ...
  c--;
    ...
    generateVehicleToken = function () {
      ...
      $.ajax({
          type: 'POST',
          url: TSMServiceUrl + "/ImportBookVehicleSlotDetailsCB",
